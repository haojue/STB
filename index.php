...
<link rel="stylesheet" href="ghbutton/gh-buttons.css">
...
echo "<h3 style='background-color:#EEEEEE;text-align:left;height:5%;width:80%'> &nbsp Usage Statistics</h3>";
include 'highchart.php';

$user = $_COOKIE["user"]; 
setcookie("user", "$user", time()+3600);

exec("sudo ps -aux | grep  'checkjob.pl'",$arr, $sta); 
#       echo var_dump($arr);
$count1 = 0;
foreach($arr AS $value){
		if (preg_match("/\/usr\/bin\/perl \.\/checkjob\.pl/i",$value)) {
...
if($count1 === 0){
echo "##############Daemon not running, please start it before you submit job#######################\n";
 echo "<form action='kill_process.php?id=1' method='post'>". "<input type='submit' name='Submit' value='start' />" . "</form>";
}
...
<form action='kill_process.php?id="1"' method='post'><input type='submit' name='Submit' value='show mine only'/></form>
<table id="jobs" border="1" width="400" cellpadding="10">
<tr>
  <th>jobid(time)</th>
  <th>submitter</th>
...
$types=array("runbps2.pl"=>"bps","runixiaewf.pl"=>"ixiaewf","runixiascreen.pl"=>"ixiascreen","runtc.pl"=>"testcenter");
...
#for ($i=0; $i <= count($jobs1) -1; $i++) 
{
  ...
  $job = array_pop($jobs1);    
  echo "<tr>";
  $traffictype =  $job->getAttribute("cmd");
  preg_match("/.*?\.\/(\w+\.pl).*/is", $traffictype, $matches);
  $want = $matches[1];
  $type = $types[$want]; 
  ...
  $params = $job->getAttribute( "params" );
    ...
    if((!empty($id))&&(!empty($submitter))&&(!empty($dev))) { 
  #  if(1) { 
 ...
 echo "<td nowarp>" . "<a href='viewlog.php?id=$id' target='_blank'>" . $id . "</a>" . "<br>" . "</td>";
  echo "<td nowrap>" . $submitter . "<br>" . "</td>";
  echo "<td nowrap>" . $dev . "<br>" . "</td>";
  echo "<td nowrap>" . $type . "<br>" . "</td>";
  echo "<td nowrap>" . "<span style='font-size: 7pt'>" . $serverip . "<br>" . "</td>";
  echo "<td nowrap>" . "<span style='font-size: 7pt'>" . $processid . "<br>" . "</td>";
  echo "<td nowrap>" . "<span style='font-size: 7pt'>"  . $params . "<br>" . "</td>";
 echo "<td nowarp>" . "<a href='viewrsi.php?id=$id' target='_blank'>" . "View RSI" . "</a>" . "<br>" . "</td>";
#  echo "<td nowarp>" . "<a href='$dev' target='_blank'>" . RSI . "</a>" . "<br>" . "</td>";
  echo "<td nowarp>" . "<form action='index.php?id=$id' method='post'>". "<input type='submit' name='Submit' value='rerun' />" . "</form>" . "</td>"; 
  echo "<td nowarp>" . "<form action='commitconfig.php?type=$type&dev=$dev' method='post'>". "<input type='submit' name='Submit' value='Reuse' />" . "</form>" . "</td>"; 
  #  echo "<td>" . $params . "<br>" . "</td>";
  echo "</tr>";
    } else {
    $i += 1;
