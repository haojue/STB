...
function loadXMLDoc()
{
	...
	if (window.XMLHttpRequest)
	 ...
	 <?php exec("sudo rm -f $_REQUEST[log]")?>;
         <?php exec("sudo wget http://10.208.130.44/$_REQUEST[log]")?>;
         <?php # exec("sudo mv -f $_REQUEST[log] bpstestresult")?>;
...
list($tclname,$tclserver,$chassisip,$profilename,$sustaintime,$cps,$port1,$port2 ,$sustaintime,$rampuptime,$rampdowntime) =  explode(",",$param);

echo "$tclname running on server $tclserver with chassis ip $chassisip, card $cardname, port $port1 and $port2";
