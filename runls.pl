...
my $session= $ARGV[6];
my $jobid = $ARGV[7];
print "#########  $tclserver $tclname $chassisip $user $pass $library $session $jobid\n\n\n";
my $timeout = 40;
my $timeoutlong = 20;
...
$exp->send("nohup ./$tclname $chassisip $lsuser $lspass \"$library\" \"$session\" > /var/www/html/$tclname.$jobid.results &\n") if($exp->expect($timeout,']#'));
$exp->send("\n") if ($exp->expect($timeout,']#'));
my $read = $exp->before();
print "read is $read\n";
